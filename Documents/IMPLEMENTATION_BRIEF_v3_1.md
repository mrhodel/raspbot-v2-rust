...
This brief is a practical handoff summary for implementation. If there is a conflict, follow `requirements_v5_1.md`.
...
- **Classical robotics** is used for motion, mapping, control, safety, behavior management, and live human-observable visualization
...
### 13. A lightweight GUI/network bridge should exist
A small read-only live visualization bridge should expose map, pose, frontier, executive state, and health data during bring-up and debug.
...
Executive, Safety, Telemetry, and a read-only visualization bridge span the full stack
```
...
Create a compilable Rust workspace that matches the architecture in `requirements_v5_1.md`.
...
- expose live read-only debug visualization data
...
7. `ui_bridge`
8. `hal` stubs
9. Yahboom I2C protocol capture/documentation
10. IMU integration
11. camera pipeline
12. event-driven depth
13. pseudo-lidar extraction
14. micro-SLAM prototype
15. occupancy mapping
16. classical frontier detection + baseline heuristics
17. planner + controller
18. safety layer
19. fast simulator
20. RL frontier selector
21. real robot integration
22. simulator/perception refinement
...
### UI Bridge
Implement a small read-only WebSocket bridge first.
...
Keep it optional and non-critical to autonomy.
...
- view live map/pose/state data in a simple browser-based UI
